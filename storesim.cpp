...
//  Version   : v1.0 09/11/17
//  Purpose   : To create a simulated queue with randomized input/output
//--------------------------------------------------------------------
...
//#endif 				 //   time that each customer arrived and
	...
	while (minute != simLength) {
        ...
        ++minute;                                           //incriment time by 1
        
        if(!custQ.isEmpty()) {
            
            waitTime = minute - custQ.dequeue();            //dequeue returns the time they entered the queue
            totalWait += waitTime;
        ...
        if(randNum == 1) {
            ...
            custQ.enqueue(minute);                          //adds someone to the line, keeping track of when they entered
            ++numArrivals;
        }
        else if(randNum == 2) {
